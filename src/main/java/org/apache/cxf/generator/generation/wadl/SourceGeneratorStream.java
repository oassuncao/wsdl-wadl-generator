...
import org.apache.cxf.tools.wadlto.jaxrs.ValidationException;
 ...
 * @since 1.0
 */
public class SourceGeneratorStream implements WriterResult {
// ------------------------------ FIELDS ------------------------------
    ...
    private static final Logger LOG = LogUtils.getL7dLogger(SourceGeneratorStream.class);
    ...
    private static final Set<Class<?>> OPTIONAL_PARAMS =
            new HashSet<>(Arrays.<Class<?>>asList(QueryParam.class,
                    HeaderParam.class,
                    MatrixParam.class,
                    FormParam.class));
...
// --------------------------- CONSTRUCTORS ---------------------------
    ...
    public SourceGeneratorStream() {
        this(new ArrayList<GeneratorResult>());
    }

    public SourceGeneratorStream(List<GeneratorResult> results) {
...
// -------------------------- OTHER METHODS --------------------------
    ...
    private GrammarInfo generateSchemaCodeAndInfo(Application app, Set<String> typeClassNames,
        ...
        if (!skipSchemaGeneration && schemaElements != null && !schemaElements.isEmpty()) {
            ...
            JAXBUtils.JCodeModel codeModel = createCodeModel(schemaElements, typeClassNames);
            if (codeModel != null) {
    ...
    private JAXBUtils.JCodeModel createCodeModel(List<SchemaInfo> schemaElements, Set<String> type) {
        JAXBUtils.SchemaCompiler compiler = createCompiler(type);
    ...
    private void generateClassesFromSchema(JAXBUtils.JCodeModel codeModel, File src) {
        try {
            CustomOutputStreamCodeWriter codeWriter = new CustomOutputStreamCodeWriter(new ByteArrayOutputStream(), encoding == null ? "UTF-8" : encoding, results);
    ...
    private void createJavaSourceFile(File src, QName qname, StringBuilder sbCode, StringBuilder sbImports,
        ...
        GeneratorResult result = new GeneratorResult(namespace, qname.getLocalPart(), "java", stream);
        results.add(result);
    }
